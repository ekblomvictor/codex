# Checklista Victor — före fredag
## 🔴 Måste (tis–ons, ~2 h)
1. **EUR-Lex:** läs **2022/191 art. 1.1** och avgör ≤6 mm-frågan slutligt **+ bekräfta i samma stycke: coach screws uteslutna, fristående muttrar ej i scope** *(brick-återbetalningen i 1788 och 2025/2153-koderna 7318 15 42/48 är redan dubbelverifierade — endast proceduren för återbetalningsansökan kvarstår att slå upp)*
2. **EUR-Lex:** bilaga I till 2023/956 — ögna raderna **7325/7326** (bekräfta vår slutsats: 7326 helt, 7325 ej)
3. **IR 2025/2621 + 1740-rättelsen:** slå upp **exakt defaultvärde för 7318 15 / Kina** (Excel publicerad; intervallet är 1,37–8,23 beroende på land)
4. **Tulltaxan/TARIC:** satser + AD-tilläggskoder för 7318 15-numren
5. **OJ C-serien:** hämta OJ-referensen för *notice of impending expiry* (utgång **18/2 2027**; notisen finns enligt MLex)
## 🟠 Bör (~1,5 h)
5b. **tullverket.se, 15 min spot-check**: självrättelse/tulltillägg (sats + lindring) och öppna e-tjänsten **"Statistik import"** en gång så du sett exportformatet *(tjänstens existens är verifierad; tullager-mekaniken likaså — kvar är bara tulltillägg-detaljerna)*
5c. Hämta och skumma **EFDA:s CBAM-kostnadspapper 2026** (branschröst, citerbar)
6. Köp **årsredovisningen 2024** (Bolagsverket ~250 kr): varulager + bruttomarginal ersätter våra härledningar; kolla om FY2025 lämnats
7. **Bolagsverkets e-tjänst verklig huvudman** (gratis): Johns ägarband
8. LinkedIn: John + Magnus + ESMA-sidan (tenure, aktivitet, rekryteringar)
9. Läs själv: esma.se CBAM-artikeln, logistiksidan, fasteners-sidan — **kopiera EXW-meningen ordagrant in i manuset** ("ni skriver ordagrant att …" slår varje parafras)
## 🟡 Kan
10. Bufab ÅR 2025, sök "CBAM" (peer-röst) · EBTI-sök på 7318 · skumma 2026/1384
## Fredag 12.45
Liveguide på andra skärmen · timer 13.20 · läs 05-DONT-SAY en sista gång · de fyra fälten redo att mejlas · **uppföljningsmejlet förskrivet** (mall i 04-DATA-REQUEST)
